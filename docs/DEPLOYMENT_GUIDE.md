# Cloudflare Pages 公開手順ガイド

このドキュメントでは、開発した `urbanity-map` (React + Vite) を Cloudflare Pages で公開する手順についてまとめます。以前は GitHub Pages を使用していましたが、現在は Cloudflare Pages (`urbanity-map.pages.dev`) に移行しています。
